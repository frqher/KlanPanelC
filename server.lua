...
function onCreateGroup(name)
	if getPlayerGroup(source) then
		outputChatBox("(Klan) Zaten klanın var.", source,255, 40, 0,true)
	elseif (getPlayerMoney(source) or 0) < 0 then
		outputChatBox("(Klan) Klanı oluşturmak için 10.000$ sahip olmalısınız.",source, 255, 20, 0,true)
	elseif isHasSpace(name) then
		outputChatBox("(Klan) Klan isminde boşluk kullanamazsınız.",source, 255, 40, 0,true)
	elseif not isASCII(name) then
		outputChatBox("(Klan) Klan isminde sadece ingilizce harfler kullanılabilir.",source, 255, 40, 0,true)
	else
...
function CreateGroup(player, name)
	if not IsGroupExists(name) then
		...
		outputChatBox("(Klan) "..name.." başarıyla oluşturuldu.",player, 0, 255, 0, true)
	...
	outputChatBox("(Klan) Klanı oluşturmak için yeterli paranız yok !", player, 255, 0, 0,true)
	end
	else
		outputChatBox("(Klan) "..name.." adlı klan zaten var.", player,255, 0, 0,true)
	end
end
		...
		if doesPlayerHavePermission(player, 13) then
			if r and g and b then
				if r <= 255 and r >= 0 and g <= 255 and g >= 0 and b <= 255 and b >= 0 then
					...
					outputChatBox("[Klan] "..string.format("#%.2X%.2X%.2X", r, g, b).."Tag rengi başarıyla değiştirildi.",player, r, g, b,true)
					...
					outputChatBox("[Klan] 0-255 Arası bir sayı olmalıdır.", player,255, 255, 0,true)
				end
				else
				outputChatBox("[Klan] Sözdizimi: /tagrenk [r] [g] [b]",player, 255, 255, 0,true)
				outputChatBox("[Klan] Örnek: /tagrenk 255 0 0", player,0, 250, 150,true)
			end
		else
			outputChatBox("[Klan] Tag rengini değiştirme izniniz yok.",player, 255, 0, 0,true)
		end
	end
end
)
		...
		if doesPlayerHavePermission(player, 13) then
			if r and g and b then
				if r <= 255 and r >= 0 and g <= 255 and g >= 0 and b <= 255 and b >= 0 then
					...
					outputChatBox("[Klan] "..string.format("#%.2X%.2X%.2X", r, g, b).."Sohbet rengi başarıyla değiştirildi.",player, r, g, b,true)
				...
				outputChatBox("[Klan] Sözdizimi: /sohbetrenk [r] [g] [b]",player, 255, 255, 0,true)
				outputChatBox("[Klan] Örnek: /sohbetrenk 255 0 0",player, 0, 250, 150,true)
			end
		else
			outputChatBox("[Klan] Sohbet rengini değiştirme izniniz yok.",player, 255, 0, 0,true)
		end
	end
end
)
...
addEventHandler("KlanKapasite", root, function(sayi, para)
	...
	dbExec(db, "UPDATE groups SET members_limit = ? WHERE group_name = ?", sayi+MemberTotal, Group)
		...
		outputChatBox("[Klan] Klan kapasitesini arttıracak paran yok.",source, 255, 0, 0,true)
	end
end)
			...
			if doesPlayerHavePermission(player, 12) then
				if r and g and b then
					if r <= 255 and r >= 0 and g <= 255 and g >= 0 and b <= 255 and b >= 0 then
						...
						outputChatBox("[Turf] "..string.format("#%.2X%.2X%.2X", r, g, b).."Turf rengi başarıyla değiştirildi.",player, r, g, b,true)
						...
						outputChatBox("[Turf]  0-255 Arası bir sayı olmalıdır.",player, 255, 255, 0,true)
					end
				else
					outputChatBox("[Turf] Sözdizimi: /turfrenk [r] [g] [b]",player, 255, 255, 0,true)
					outputChatBox("[Turf] Örnek: /turfrenk 255 0 0", player,0, 250, 150,true)
				end
			else
				outputChatBox("[Turf] Turf rengini değiştirme izniniz yok.",player, 255, 0, 0,true)
			end
		end
	end
end)
...
function (Red, Green, Blue)
	...
	if Group then
		if doesPlayerHavePermission(source, 13) then
			...
			dbExec(db, "UPDATE groups SET tag_color = ? WHERE group_name = ?", color, Group)
			outputChatBox("[Klan] "..string.format("#%.2X%.2X%.2X", Red, Green, Blue).."Tag rengi başarıyla değiştirildi.",source, Red, Green, Blue,true)
			...
			outputChatBox("[Klan] Tag rengini değiştirme izniniz yok.", source, 255, 0, 0,true)
		end
	end
end
)
	...
	if not turfTimer[source] then
		...
		if Group then
			if doesPlayerHavePermission(source, 12) then
				local color = Red..", "..Green..", "..Blue
				dbExec(db, "UPDATE groups SET turf_color = ? WHERE group_name = ?", color, Group)
				outputChatBox("[Turf] "..string.format("#%.2X%.2X%.2X", Red, Green, Blue).."Turf rengi başarıyla değiştirildi.", source,Red, Green, Blue,true)
				...
				outputChatBox("#[Turf] Turf rengini değiştirme izniniz yok.", source, 255, 0, 0,true)
			end
		end
	end
end
...
local Group = getElementData(source,"Group")
	if Group then
		if doesPlayerHavePermission(source, 13) then
			local color = Red..", "..Green..", "..Blue
			dbExec(db, "UPDATE groups SET chat_color = ? WHERE group_name = ?", color, Group)
			outputChatBox("[Klan] "..string.format("#%.2X%.2X%.2X", Red, Green, Blue).."Sohbet rengi başarıyla değiştirildi.", source,Red, Green, Blue,true)
			...
			outputChatBox("[Klan] Sohbet rengini değiştirme izniniz yok.", source, 255, 0, 0,true)
		end
	end
end
)
...
function (Player)
	...
	if Player then
		...
		if getGroupFounderAccount(mgroup) == getAccountName(getPlayerAccount(Player)) then outputChatBox("(Klan)  Klan Sahibine mute atamazsın !",source, 255, 0, 0,true) return end
	...
	if money and groupName then
		if doesPlayerHavePermission(source, 10) then
			...
			if balance and pMoney >= money and balance+money > 0 then
				...
				outputChatBox("(Klan) Yeteri kadar paran yok.", source, 0, 255, 0,true)
			end
		else
			outputChatBox("(Klan) Klan bankasına para yatırma izniniz yok.", source, 255, 0, 0,true)
		end
	end
end)
	...
	if money and groupName then
		if doesPlayerHavePermission(source, 11) then
			...
			if balance and balance-money >= 0 then
				...
				outputChatBox("(Klan) Klan bankanızda bu kadar paran yok.", source, 0, 255, 0,true)
			end
		else
			outputChatBox("(Klan) Klan bankasından paraçekme izniniz yok.", source, 255, 0, 0,true)
		end
	end
end)
		...
		if doesPlayerHavePermission(source, 17) then
			if Group == ItemGroup then
				if name4 ~= name2 then
					...
					outputChatBox("(Klan) Parayı kendine veremezsin !", source,255, 0, 0,true)
				end
			else
				outputChatBox("(Klan)  Klanda Bu İsimde Bir Oyuncu Yok.",source, 255, 0, 0,true)
			end
		else
			outputChatBox("(Klan) Klan üyelerine Para Verme izniniz yok.",source, 255, 0, 0,true)
		end
	end
end
)
...
function(Account, level, oLevel, Reason)
	...
	if groupName then
		if doesPlayerHavePermission(source, 14) then
			if getGroupFounderAccount(groupName) == Account then outputChatBox("(Klan)  Klan sahibini uyaramazsınız !", source, 255, 0, 255,true) return end
			...
			outputChatBox("(Klan) Üyeleri uyarma izniniz yok.", source, 255, 0, 0,true)
		end
	end
end)
			...
			if type(result) == "table" and #result ~= 0 then
				...
				for i, R in pairs(result) do
					...
					if R["group_rank_name"] == addAfterRank then
						...
						outputChatBox(source, "(Klan) "..rankName.." adlı rütbe başarıyla oluşturuldu!",source, 0, 255, 0,true)
	...
	if groupName and rankType == 1 then
		...
		if type(result) == "table" and #result ~= 0 then
			if result[1]["rank_type"] == 3 then
				...
				outputChatBox("(Klan) "..rankName.." adlı rütbe başarıyla kaldırıldı!", source, 0, 255, 0,true)
				...
				outputChatBox("(Klan) Bu rütbe kaldırılamaz.",source, 255, 40, 0,true)
			end
		end
	end
end)
...
function(rankName, newRankName, permission)
	...
	if groupName and rankType == 1 then
		...
		outputChatBox("(Klan) "..rankName.." adlı rütbe başarıyla düzenlendi!", source, 0, 255, 0, true)
	...
	if groupName then
		...
		elseif rankType == 1 then
			...
			if type(result) == "table" and #result == 1 then
				outputChatBox("(Klan) Klandan ayrılamazsın çünkü Kurucusun ", player,255, 40, 0,true)
			else
				local result = dbPoll(dbQuery(db, "SELECT group_members FROM groups WHERE group_name = ?", groupName), -1)
				local cont = result[1]["group_members"] - 1
				dbExec(db, "DELETE FROM group_members WHERE member_account = ?", getAccountName(getPlayerAccount(player)))
...
function GroupBlockAccount(Group, by, account, Reason)
	if IsGroupExists(Group) then
		...
		if isAccountBlocked(Group, account) then
			outputChatBox("(Klan) The Account Is Already Blocked.",source,255,255,0,true)
		else
			...
			outputChatBox("(Klan) The Account Was Blocked Successfully.",source,255,255,0,true)
		end
	end
end
...
function (theLogID, theLogTime)
	if doesPlayerHavePermission(source, 18) then
		...
		outputChatBox("(Klan) İşlem başarıyla silindi",source, 255, 255, 0,true)
		...
		outputChatBox("(Klan) İşlemi silme izniniz yok.", source,255, 255, 0,true)
	end
end
)
...
function (GroupName, AccountName, AccountPassword)
	if not doesPlayerHavePermission(source, 7) then
		outputChatBox("(Klan) Klanı silme yetkiniz yok.", source,255, 0, 0,true)
	elseif not IsGroupExists(GroupName) then
		outputChatBox("(Klan) Klan adı hatalı.", source,255, 0, 0,true)
	elseif not getAccount(AccountName) then
		outputChatBox("(Klan) Kullanıcı adı hatalı.", source,255, 0, 0,true)
	elseif not getAccount(AccountName, AccountPassword) then
		outputChatBox("(Klan) Kullanıcı adı ve Şifre eşleşmiyor.", source,255, 0, 0,true)
	elseif getElementData(source, "Group") ~= GroupName then
		outputChatBox("(Klan) Klan adı eşleşmiyor.",source, 255, 0, 0,true)
	else
		for i, player in ipairs(getElementsByType("player")) do
			if getElementData(player, "Group") == GroupName then
	...
	if cmd == "Yükselt" and groupName then
		if doesPlayerHavePermission(source, 2) then
			if accountName == Account then
				outputChatBox("(Klan) Kendi rütbeni yükseltemezsin.",source, 255, 0, 0,true)
			else
			...
			outputChatBox("(Klan) Rütbe yükseltme yetkiniz yok.", source, 255, 0, 0,true)
		end
	elseif cmd == "Düşür" and groupName then
		if doesPlayerHavePermission(source, 1) then
			if accountName == Account then
				outputChatBox("(Klan) Kendi rütbeni düşüremezsin.",source, 255, 0, 0,true)
			elseif newRankName == "Klandan At" then
				if founderaccount == Account then outputChatBox("(Klan)  Klan sahibini klandan atamazsın", source,255, 0, 0,true) return end
				if doesPlayerHavePermission(source, 3) then
					...
					outputChatBox("(Klan) Üyeyi klandan atmak için yetkiniz yok.",source, 255, 0, 0,true)
				end
			else
				if founderaccount == Account then outputChatBox("(Klan)  Klan sahibinin rütbesini düşüremezsin", source,255, 0, 0,true) return end
				dbExec(db, "UPDATE group_members SET member_rank = ? WHERE group_name = ? AND member_rank = ? AND member_account = ?", newRankName, groupName, currentRankName, Account)
			...
			outputChatBox("(Klan) Rütbe düşürme yetkiniz yok.", source, 255, 0, 0,true)
		end
...
function(name, NewLimit)
	if IsGroupExists(name) then
		...
		if CurrentMember <= tonumber(NewLimit) then
			...
			outputChatBox("[Klan Yönetimi] Üye limiti geçerli şuanki üye limitinden düşük olamak.",source, 255, 0, 0,true)
		end
	else
		outputChatBox("[Klan Yönetimi] Klan mevcut değil.",source, 255, 0, 0,true)
	end
end)
...
function(name, NewName)
	if IsGroupExists(name) then
		if isHasSpace(NewName) then
			outputChatBox("[Klan Yönetimi] Klan isminde boşluk bırakmayın.",source, 255, 40, 0,true)
		elseif not isASCII(NewName) then
			outputChatBox("[Klan Yönetimi] Klan isminde sadece ingilizce harfler olmalıdır.",source, 255, 40, 0,true)
		elseif IsGroupExists(NewName) then
			outputChatBox("[Klan Yönetimi] "..NewName.." adlı klan zaten var.", source,255, 0, 0,true)
		else
...
function(GroupName)
	if not IsGroupExists(GroupName) then
		outputChatBox(source, "[Klan Yönetimi] Klan mevcut değil.",source, 255, 0, 0,true)
	else
		dbExec(db, "DELETE FROM groups WHERE group_name = ?", GroupName)
		dbExec(db, "DELETE FROM group_ranks WHERE group_name = ?", GroupName)
		dbExec(db, "DELETE FROM group_members WHERE group_name = ?", GroupName)
...
function(name, Text)
	if IsGroupExists(name) then
		dbExec(db, "UPDATE groups SET group_info = ? WHERE group_name = ?", Text, name)
		outputChatBox("[Klan Yönetimi] Klan Bilgisi Başarıyla Güncellendi!",source, 0, 255, 0,true)
	else
		outputChatBox("[Klan Yönetimi] Klan mevcut değil.", source,255, 0, 0,true)
	end
end)
