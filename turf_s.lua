...
function()
	...
	for i = 1, #Table do
		if Table[i][6] == Group or Table[i][6] == "Gta-IS" then
			...
			outputChatBox("#FF9600[Turf] #74B3FFEn yakın Turf bölgenizde doğdunuz.", source,	0, 255, 255);
			break
		end
	end
end );
	...
	if gMoneyTime == 30 then
		...
		for G, X in pairs(getGroupsTurf(50)[1]) do
			...
			for i, player in ipairs(OnlineMember) do
					outputChatBox("[Turf] Bölgelerinizden $"..MoneyPerOnlineMember.." kazandınız.", player, 219, 0, 0, true)
					outputChatBox("[Turf] Bölgelerinizden "..X.." puan kazandınız.", player, 219, 0, 0, true)
	...
	for Col, Area in pairs(RadarAreaColTurf) do
		for i, player in ipairs(getElementsWithinColShape(Col, "player")) do
			...
			if getElementData(player, "TurfStat2") then
				...
				local Group = player:getData("Group");
				if Group then
					if getElementData(player,"BandTurf") then outputChatBox("[Turf] Bölge alamazsınız çünkü yasaklandınız !", 219, 0, 0, true) return end
					if not isPedInVehicle(player) then
						...
						if z < 45 and z >= 1 and not doesPedHaveJetPack(player) then
							...
							if (Group ~= oA and LA >= 50 and isTurfAlliances(player, oA)) or (Group ~= oB and LB >= 50 and isTurfAlliances(player, oB)) then
								outputChatBox("[Turf] İttifak klanın bölgesini alamazsınız!", player, 219, 0, 0, true);
							else
							...
							if RadarAreasTurf[Area]["WRNNING"][Group] == nil then
								if oA ~= Group and LA > 50 and LA < 75 then
									...
									for i, p in ipairs(getGroupOnlineMember(oA)) do
										outputChatBox("[Turf] Uyarı!! Bölgenizden birisi saldırı altında ! "..Group..".", p, 219, 0, 0, true);
									end
							...
							if LA >= 55 and oA == Group then
								if LA == 55 then
									...
									for i, player in ipairs(getGroupOnlineMember(Group)) do
										outputChatBox("[Turf] Artık bölge sizin kontrolünüzde!", player, 219, 0, 0, true);
										outputChatBox("[Turf] Çevrimiçi Klan üyeleri her 5 dakikada bir $400 kazanacak.", player, 219, 0, 0, true);
							...
							elseif LB >= 55 and oB == Group then
								if LB == 55 then
									for i, player in ipairs(getGroupOnlineMember(Group)) do
										triggerEvent("onGroupTurf", player, Group);
										outputChatBox("[Turf] Artık bölge sizin kontrolünüzde!", player, 219, 0, 0, true);
										outputChatBox("[Turf] Çevrimiçi Klan üyeleri her 5 dakikada bir $2000 kazanacak.", player, 219, 0, 0, true);
										playSoundFrontEnd(player, 101);
									end
							...
							outputChatBox(player, "[Turf] Bu yükseklikten bölge alamazsınız, Lütfen yere inin !", player, 219, 0, 0, true);
						end
					else
						outputChatBox(player, "[Turf] Araçtayken bölge alamazsınız !", player, 219, 0, 0, true);
					end
				else
					outputChatBox("[Turf] Bölge ele geçirmek için bir klana katılın veya bir klan oluşturun !", player, 219, 0, 0, true);
				end
			end
			local oA = RadarAreasTurf[Area]["Loyalty"][1][1]
			local LA = RadarAreasTurf[Area]["Loyalty"][1][2]
